...
## 外部ツール連携 (n8n, スマホ連携等)
デプロイした「ウェブアプリのURL」に対してPOSTリクエストを送ることで、外部から直接データを登録したりAIを呼び出すことができます。

- **新規の議事録を自動登録する**: `{ "action": "saveMeeting", "payload": { "client": "...", "related": "...", "summary": "...", "fulltext": "..." } }`
- **外部からAIを呼び出す**: `{ "action": "processQuery", "payload": { "query": "明日のA社の商談で気をつけることは？", "mode": "clone" } }`
